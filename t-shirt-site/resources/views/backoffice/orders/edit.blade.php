...
@section('content')
    ...
    <form action="{{ route('backoffice.orders.update', $order->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <textarea class="form-control" id="address" name="address" rows="2">{{ old('address', $order->address) }}</textarea>
        </div>
